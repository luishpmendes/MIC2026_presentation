...
%----------------------------------------------------------------------------------------
\begin{frame}
\frametitle{Why PCI planning is hard in practice}

\begin{columns}[T,onlytextwidth]
\column{0.62\textwidth}
\begin{itemize}
\item \textbf{Every LTE/5G cell broadcasts a Physical Cell Identity (PCI)} so user equipment can detect and distinguish cells.
\item \textbf{The PCI space is limited} (LTE: $504$; 5G: $1008$) $\Rightarrow$ reuse is unavoidable.
\item Reuse must avoid \textbf{technological conflicts} (collision, confusion, and critical modulo-$k$ interference).
\item Operators also need \textbf{operational safety}: staged rollouts mean that a good final plan can still fail due to \emph{PCI shuffles}.
\end{itemize}

\vspace{0.35em}
\small
\textit{We model this as a constrained multi-objective optimization problem and approximate its Pareto front.}

\column{0.38\textwidth}
\centering
% \includegraphics[width=\textwidth]{figs/fig_hexgrid.png}
\end{columns}

% Speaker note: Keep this slide high-level. The next slide defines each conflict with a visual.
\end{frame}

%----------------------------------------------------------------------------------------
\begin{frame}
\frametitle{Conflicts to avoid (technology + rollout)}

\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
\begin{itemize}
\item \textbf{Collision (hard):} neighbors share the same PCI $\rightarrow$ UE cannot distinguish signals.
\item \textbf{Confusion (hard):} a cell has two distinct neighbors with the same PCI $\rightarrow$ ambiguous handover target.
\item \textbf{Modulo-$\mathbf{k}$ collision:} neighbors have PCIs congruent modulo $k$ $\rightarrow$ reference signals interfere (critical on \emph{top neighbors}).
\item \textbf{PCI shuffle:} batched rollout assigns a new PCI equal to a neighbor's \emph{current} PCI $\rightarrow$ transient collision/confusion and deployment failure.
\end{itemize}

\column{0.45\textwidth}
\centering
\begin{tabular}{@{}cc@{}}
% \includegraphics[width=0.48\textwidth]{figs/fig_collision.png} &
% \includegraphics[width=0.48\textwidth]{figs/fig_confusion.png} \\
% \includegraphics[width=0.48\textwidth]{figs/fig_modk_clique.png} &
% \includegraphics[width=0.48\textwidth]{figs/fig_shuffle.png} \\
\end{tabular}
\end{columns}

% Speaker note: Map each visual to the constraints/objectives on the next section (hard vs optimized).
\end{frame}
...
%----------------------------------------------------------------------------------------
\begin{frame}
\frametitle{Network model and neighborhoods}

\begin{columns}[T,onlytextwidth]
\column{0.64\textwidth}
We model the network as an undirected graph $G = (V,E)$:
\begin{itemize}
\item $V$: cells; $E$: neighbor/interference relations.
\item The edge set is partitioned: $E = T \cup N,\; T \cap N = \emptyset$.
\begin{itemize}
\item $T$: \textbf{top neighbors} (critical relations) where modulo-$k$ collisions are \emph{forbidden};
\item $N$: regular neighbors where direct collisions are \emph{forbidden}, while modulo-$k$ collisions are \emph{minimized}.
\end{itemize}
\item Second-level neighbor pairs (used to model confusions):
\[
...
\centering
\begin{tikzpicture}[
node/.style={circle,draw,inner sep=1.2pt},
Eedge/.style={thick},
Tedge/.style={thick},
Sedge/.style={thick,dashed},
scale=1.0
]
\node[node] (u) at (0,1.0) {\scriptsize $u$};
\node[node] (v) at (1.4,0.0) {\scriptsize $v$};
\node[node] (w) at (0,-1.0) {\scriptsize $w$};

\draw[Tedge] (u) -- (v);
\draw[Eedge] (v) -- (w);
\draw[Sedge] (u) -- (w);

\node[above=0.05cm of u] {\scriptsize top neighbor $(u, v) \in T$};
\node[below=0.05cm of w] {\scriptsize regular neighbor $(v, w) \in N$};
\node[left=0.25cm of v] {\scriptsize second-level pair $(u, w) \in S$};
\end{tikzpicture}

\vspace{0.25em}
{\scriptsize Dashed edges are \emph{not} in $E$ (they represent confusion pairs).}
\end{columns}
