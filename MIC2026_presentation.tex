...
\begin{itemize}
\item Each Radio Access Network (RAN) cell must broadcast a \textbf{Physical Cell Identity (PCI)} so User Equipments (UEs) can detect and distinguish cells.
...
\item Operators also care about \textbf{operational objectives}: minimize disruptive PCI changes (PCI shuffles) and avoid deployment failures during staged rollouts.
\end{itemize}
...
\begin{itemize}
\item Neighboring cells must have different PCIs.
Otherwise, a \textbf{collision} occurs, and the UE cannot connect to either cell.
For example, UE in the figure cannot connect to a cell because both cells A and B have the same PCI 10.
\item Also, due to constraints in the primary and second sync signal and the 5G numerology, it is very desirable that neighboring PCIs be different in modules.
\end{itemize}

\column{0.38\textwidth}
\centering
\begin{figure}
\includegraphics[width=0.99\textwidth]{figs/collision.pdf}
\end{figure}
\end{columns}
...
\column{0.62\textwidth}
\begin{itemize}
\item However, it is extremely hard to ensure module-$3$ and module-$4$ assignments in large production networks.
\item The reason is that it is very common to have a cluster of size $\ge 5$ in which all cells are neighbors of the other cells with the cluster.
\item Precisely speaking, if we model these cells as nodes and the neighborhood relationships as edges in a neighboring graph, it is very common to find cliques of size $5+$.
\end{itemize}

\column{0.38\textwidth}
\centering
\begin{figure}
\includegraphics[width=0.99\textwidth]{figs/clique.pdf}
\end{figure}
\begin{figure}
\includegraphics[width=0.99\textwidth]{figs/top_neighbors.pdf}
\end{figure}
\end{columns}
...
\column{0.62\textwidth}
\begin{itemize}
\item \textbf{Confusion} occurs when a cell has two neighbors with the same PCI.
When a UE asks for handover, the base station does not know to which cell itshould hand the UE over.
\item In other words, if \textbf{second-level neighbors (neighbor of neighbors)} have thesame PCI that the current cell, confusion may occur.
\item Example: The UE is moving from cell A to cell C.
Then, the UE asks Cell A to hand the connections over to the cell with PCI $10$.
But now, Cell A cannot determine whether UE refers to Cell B or Cell C since they have the same PCI.
\end{itemize}

\column{0.38\textwidth}
\centering
\begin{figure}
\includegraphics[width=0.99\textwidth]{figs/confusion.pdf}
\end{figure}
\end{columns}
...
\begin{itemize}
\item When scheduling batches, a \textbf{PCI shuffle} may occur: a node receives the PCI number previously assigned to a neighbor or a second-level neighbor, which is assigned to another batch.
\item Since the batches are deployed in sequence, we may find collisions and/or confusions, and therefore, the deployment will not be validated.
\end{itemize}
...
\column{0.62\textwidth}
\begin{itemize}
\item Suppose we have two neighboring cells A and B, which PCIs must be changed.
\item The original PCI for cell A is $100$, and for cell B is $150$.
\item Now, let us suppose that we must assign PCI $150$ for cell A.
\item Since cells A and B are in different batches, we cannot deploy batch 1 first because it will cause a PCI collision with cell B, even though the PCI for B should be changed later on.
\item To avoid PCI shuffle, we must proper order the batches for delivery.
\end{itemize}

\column{0.38\textwidth}
\centering
\begin{figure}
\includegraphics[width=0.99\textwidth]{figs/PCI_shuffle_1.pdf}
\end{figure}
\end{columns}
...
\column{0.62\textwidth}
\begin{itemize}
\item Here, PCI $150$ must be assigned to cell A and PCI $100$ must be assigned to cell B (crossed PCI shuffle).
\item In this case, it does not matter the other we try to deploy the batches because collisions will happen in any case.
\item To avoid PCI shuffle we must assign dependent nodes (for which we must change the PCIs) in the same batch.
\item However, each batch has a limited size or capacity.
\item Therefore, it can be challenging to find a proper packing.
\end{itemize}

\column{0.38\textwidth}
\centering
\begin{figure}
\includegraphics[width=0.99\textwidth]{figs/PCI_shuffle_2.pdf}
\end{figure}
\end{columns}
